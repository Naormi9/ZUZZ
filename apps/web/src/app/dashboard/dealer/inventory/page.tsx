...
const STATUS_COLORS: Record<string, string> = {
  ...
  paused: 'bg-yellow-100 text-yellow-700', sold: 'bg-blue-100 text-blue-700',
  archived: 'bg-gray-100 text-gray-600', pending_review: 'bg-amber-100 text-amber-700',
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/cars/create">
          <Button className="gap-2"><Plus className="h-4 w-4" />מודעה חדשה</Button>
        </Link>
      </div>

      {/* Status filters */}
      <div className="flex flex-wrap gap-2 mb-6">
        {['', 'active', 'draft', 'paused', 'sold', 'pending_review', 'rejected', 'archived'].map((s) => (
          <button
            key={s}
      ...
      {loading ? (
            ...
            <Card key={listing.id}>
              <CardContent className="p-3 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 truncate">{listing.title || 'ללא שם'}</p>
                ...
                <div className="flex items-center gap-4 text-xs text-gray-400">
                  ...
                  <span className="flex items-center gap-1"><Users className="h-3 w-3" />{listing._count?.leads || 0}</span>
                </div>
                <Link href={`/cars/${listing.id}`}>
                  <Button size="sm" variant="outline">צפייה</Button>
                </Link>
              </CardContent>
            </Card>
          ))}
        ...
        <div className="text-center py-16 text-gray-500">
          ...
          <Link href="/cars/create"><Button variant="outline" className="mt-4">פרסם מודעה ראשונה</Button></Link>
        </div>
      )}
    </div>
  );
