...
export class MockMapProvider implements MapProvider {
  async geocode(address: string): Promise<GeocodeResult | null> {
    ...
    const match = ISRAELI_CITIES.find(
      (c) => normalized.includes(c.name.toLowerCase()) || address.includes(c.nameHe),
    );
  ...
  private haversineDistance(a: GeoPoint, b: GeoPoint): number {
    ...
    const h =
      sinLat * sinLat + Math.cos(this.toRad(a.lat)) * Math.cos(this.toRad(b.lat)) * sinLng * sinLng;
