...
async function main() {
  ...
  const admin = await prisma.user.create({
    data: {
      ...
      isActive: true,
      profile: {
        create: { displayName: 'מנהל ZUZZ', verificationStatus: 'verified', badges: JSON.stringify([]) },
      },
    },
  });
    ...
    data: {
      ...
      carDetails: {
        create: {
          ...
          isElectric: false,
          bodyType: 'crossover',
          features: ['מולטימדיה', 'מצלמה אחורית', 'חיישני חניה', 'שליטה מרחוק', 'בקרת שיוט אדפטיבית'],
        },
      },
      trustFactors: {
        create: [
          { type: 'verified_owner', category: 'identity', status: 'positive', weight: 1.5, score: 15, label: 'Verified Owner', labelHe: 'בעלות מאומתת' },
          { type: 'no_accident', category: 'history', status: 'positive', weight: 1.2, score: 12, label: 'No Accidents', labelHe: 'ללא תאונות' },
          { type: 'full_maintenance', category: 'history', status: 'positive', weight: 1.0, score: 10, label: 'Full Service History', labelHe: 'שירות מלא בסוכנות' },
          { type: 'test_valid', category: 'documentation', status: 'positive', weight: 1.0, score: 10, label: 'Valid Test', labelHe: 'טסט בתוקף' },
          { type: 'high_completeness', category: 'completeness', status: 'positive', weight: 0.8, score: 8, label: 'High Completeness', labelHe: 'מודעה מפורטת' },
          { type: 'single_owner', category: 'history', status: 'positive', weight: 1.0, score: 10, label: 'Single Owner', labelHe: 'יד ראשונה' },
        ],
      },
    },
  });
  ...
  const carListing2 = await prisma.listing.create({
    data: {
      ...
      description: 'טסלה מודל 3 Long Range, גרסה חדשה (Highland). Full Self-Driving. סוללה 100%. אחריות מלאה.',
      priceAmount: 219000,
      priceCurrency: 'ILS',
      isNegotiable: false,
      city: 'ראשון לציון',
      ...
      carDetails: {
        create: {
          ...
          bodyType: 'sedan',
          features: ['Full Self-Driving', 'Premium Audio', 'חימום מושבים', 'גג זכוכית', 'Autopilot'],
        },
      },
      trustFactors: {
        create: [
          { type: 'verified_dealer', category: 'identity', status: 'positive', weight: 1.5, score: 15, label: 'Verified Dealer', labelHe: 'סוחר מאומת' },
          { type: 'no_accident', category: 'history', status: 'positive', weight: 1.2, score: 12, label: 'No Accidents', labelHe: 'ללא תאונות' },
          { type: 'warranty_active', category: 'documentation', status: 'positive', weight: 1.0, score: 10, label: 'Active Warranty', labelHe: 'אחריות בתוקף' },
          { type: 'high_completeness', category: 'completeness', status: 'positive', weight: 0.8, score: 8, label: 'Complete Listing', labelHe: 'מודעה מלאה' },
          { type: 'docs_uploaded', category: 'documentation', status: 'positive', weight: 1.0, score: 10, label: 'Documents', labelHe: 'מסמכים הועלו' },
        ],
      },
    },
  });
  ...
  const carListing3 = await prisma.listing.create({
    data: {
      ...
      trustFactors: {
        create: [
          { type: 'accident_declared', category: 'history', status: 'warning', weight: 1.2, score: -8, label: 'Accident Declared', labelHe: 'דווחה תאונה' },
          { type: 'test_valid', category: 'documentation', status: 'positive', weight: 1.0, score: 10, label: 'Valid Test', labelHe: 'טסט בתוקף' },
        ],
      },
    },
  ...
  const dealerCars = [
    {
      ...
      price: 189000,
      make: 'Hyundai', model: 'Tucson', trim: 'N Line', year: 2023,
      mileage: 18000, gearbox: 'automatic', fuelType: 'hybrid',
      engineVolume: 1600, hp: 230, color: 'כחול', bodyType: 'suv',
    },
    {
      title: 'BYD Atto 3 2024 — חשמלי מלא, 0 ק"מ',
      price: 159000,
      make: 'BYD', model: 'Atto 3', year: 2024,
      mileage: 50, gearbox: 'automatic', fuelType: 'electric',
      hp: 204, color: 'לבן', bodyType: 'suv',
      isElectric: true, batteryCapacity: 60.5, rangeKm: 420,
      acChargeKw: 7, dcChargeKw: 80, connector: 'ccs',
    },
  ];
  ...
  const conv = await prisma.conversation.create({
    data: {
      ...
      messages: {
        create: [
          { senderId: buyer.id, content: 'שלום, ראיתי את הטויוטה קורולה. האם ניתן לראות את הרכב?', type: 'text' },
  ...
  await prisma.featureFlag.createMany({
    data: [
      { key: 'cars_enabled', name: 'Cars Vertical', description: 'Enable cars vertical', isEnabled: true },
      { key: 'homes_enabled', name: 'Homes Vertical', description: 'Enable homes vertical', isEnabled: true },
      { key: 'market_enabled', name: 'Market Vertical', description: 'Enable market vertical', isEnabled: true },
      { key: 'messaging_enabled', name: 'Messaging', description: 'Enable in-app messaging', isEnabled: true },
      { key: 'promotions_enabled', name: 'Promotions', description: 'Enable paid promotions', isEnabled: false },
      { key: 'ev_features', name: 'EV Features', description: 'Show EV-specific fields', isEnabled: true },
      { key: 'dealer_portal', name: 'Dealer Portal', description: 'Enable dealer portal', isEnabled: true },
      { key: 'trust_score_visible', name: 'Trust Score Display', description: 'Show trust scores publicly', isEnabled: true },
    ],
  });
